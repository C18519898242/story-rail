...
```mermaid
flowchart LR
    Player["玩家"] <--> CLI["CLI 游戏循环<br/>cli.py"]

    ScriptFile["JSON 剧本文件"] --> Loader["剧本加载器<br/>loader.py"]
    Loader --> Models["内存中的剧本对象<br/>models.py"]
    Models --> Engine["剧情引擎<br/>engine.py"]

    Env[".env 配置"] --> Config["配置读取<br/>config.py"]
    Config --> Factory["Provider 工厂<br/>providers/factory.py"]
    Factory --> Provider["mock / OpenAI / Anthropic"]

    CLI --> Loader
    CLI <--> Engine
    CLI --> Narrator["AI 叙事器<br/>narrator.py"]
    Narrator <--> Provider
```
